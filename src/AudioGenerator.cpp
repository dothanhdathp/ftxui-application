...
void AudioGenerator::GenerateAudioFile(audio_info data, std::string filename, IAudioGeneratorCallback* callback) {
    ...
    if(16 == data.bit_per_sample) {
        std::thread m_thread_render(&AudioGenerator::GenerateAudioTask<int16_t>, this, data, filename, callback);
        m_thread_render.detach();
        // AudioGenerator::GenerateAudioTask<int16_t>(data, filename, callback);
    } else {
        std::thread m_thread_render(&AudioGenerator::GenerateAudioTask<int32_t>, this, data, filename, callback);
        m_thread_render.detach();
    }
}
